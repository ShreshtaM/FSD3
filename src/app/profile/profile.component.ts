...
cart:string;
  ...
  ngOnInit() {
    ...
    this.city=localStorage.getItem('city');
    let url= "http://b8java28.iiht.tech:3000/showallproduct?city="+this.city;
    fetch(url,{
      method:"GET",
      headers:{
    ...
    this.urlnew = 'http://b8java28.iiht.tech:3000/findcustomerid/'+this.cid;
    fetch(this.urlnew)
    .then(res=>res.json())
    .then(data=>{
  ...
  .then(data=>{
    ...
    this.cid=localStorage.getItem('id');
    this.url=`http://b8java28.iiht.tech:3000/findcustomerid/`+this.cid;
    fetch(this.url)
    .then(res=>res.json())
    .then(data=>{
      console.log("mydata");
